...
SaveBinary::SaveBinary(const std::string& saveFilePath) : m_locked(false) {
	...
	if (!file.is_open()) {
		std::cerr << RED_TEXT << "Failed to open save file: " << saveFilePath << std::endl;
		return;
	}
	...
	if (fileSize < MIN_SAVE_SIZE) {
		std::cerr << RED_TEXT << "Save file size is too small: " << saveFilePath << ". Expected minimum size: " << MIN_SAVE_SIZE << ", Actual size: " << fileSize << std::endl;
		file.close();
		return;
	}
...
uint8_t SaveBinary::getByte(uint32_t address) const {
	if (address >= m_data.size()) {
		std::cerr << RED_TEXT << "Address out of bounds: " << std::hex << address << std::endl;
		return 0;
	}
...
void SaveBinary::setByte(uint32_t address, uint8_t value) {
	// error if locked
	if (m_locked) {
		std::cerr << RED_TEXT << "Save file is locked" << std::endl;
		return;
	}
...
void SaveBinary::Iterator::seek(uint32_t address) {
	if (m_address == address) {
		//std::cerr << RED_TEXT << "Iterator is already at the requested address: " << std::hex << address << std::endl;
		return;
	}
	m_address = address;
}
